...
outcomesRed_df <- 
  ...
  #   whole time, then the treatment was a failure
	mutate(
		Ab_eissenberg_1997 = case_when(
			completedProtocol  ~ consecNeg,
			!completedProtocol ~ FALSE
		)
	) %>% 
	select(who, Ab_eissenberg_1997) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_eissenberg_1997)
```
...
outcomesRed_df <- 
  ...
  #   over time.
	...
	mutate(
		...
		Ab_lingA_1976 = 120 - ling1976o22_use
	) %>%
	select(who, Ab_lingA_1976) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_lingA_1976)
```
  ...
  rowwise() %>% 
	...
	mutate(
	  ling1976o100_use = case_when(
		...
		Ab_lingB_1976 = 120 - ling1976o100_use
	) %>%
	select(who, Ab_lingB_1976) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_lingB_1976)
```
...
outcomesRed_df <- 
  ...
  mutate(
	...
	mutate(
		Ab_petitjean_2001 = 1 - petitjean2001_use
	) %>% 
	select(who, Ab_petitjean_2001) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_petitjean_2001)
```
...
outcomesRed_df <- 
  ...
  # Count "+" UDS; 0 could be complete dropout or all negative
	...
	mutate(Ab_shufman_1994 = 1 - shufman1994_useP) %>% 
	select(who, Ab_shufman_1994) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_shufman_1994)
```
...
outcomesRed_df <- 
  ...
  # Ignore missing UDS
	...
	mutate(Ab_soyka_2008 = 1 - soyka2008_use) %>% 
	ungroup() %>% 
	select(who, Ab_soyka_2008) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_soyka_2008)
```
...
outcomesRed_df <- 
  ...
  # Count "+" UDS; 0 could be complete dropout or all negative
	...
	mutate(Ab_strain_1993 = 1 - strain1993_use) %>% 
	select(who, Ab_strain_1993) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_strain_1993)
```
...
outcomesRed_df <- 
  ...
  # Count "+" UDS; 0 could be complete dropout or all negative
	...
	mutate(Ab_strain_1994 = 1 - strain1994_use) %>% 
	select(who, Ab_strain_1994) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_strain_1994)
``` 
...
outcomesRed_df <- 
  ...
  rowwise() %>% 
	...
	mutate(Ab_strain_1996 = 1 - strain1996_use) %>% 
	select(who, Ab_strain_1996) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_strain_1996)
```
...
outcomesRed_df <- 
  ...
  # Count "+" UDS; 0 could be complete dropout or all negative
	...
	mutate(Ab_strain_1999 = 1 - strain1999_use) %>% 
	select(who, Ab_strain_1999) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_strain_1999)
```
...
outcomesRed_df <- 
  ...
  mutate(
	...
	mutate(Ab_woody_2008 = 1 - woody2008_use) %>% 
	select(who, Ab_woody_2008) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_woody_2008)
```
...
outcomesRed_df <- 
  ...
  #   of study participation - the number of positive weeks
	mutate(Ab_zaks_1972 = str_length(udsPattern) - zaks1972_use) %>% 
	ungroup() %>% 
	select(who, Ab_zaks_1972) %>% 
	left_join(outcomesRed_df, ., by = "who")

outcomesRed_df %>% 
  filter(who %in% examplePeople_int) %>% 
  select(who, usePatternUDS, Ab_zaks_1972)
```
...
# write_csv(
#   outcomesRed_df,
#   file = "../inst/extdata/outcomes_reduction_20221123.csv"
# )
```
